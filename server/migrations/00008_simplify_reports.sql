...
-- Drop old report snapshot tables (child tables first)
...
-- Recreate reports as a simple close/reopen table
...
-- Performance index for expense queries by household and date
CREATE INDEX IF NOT EXISTS idx_expenses_household_incurred ON expenses(household_id, incurred_at);
...
DROP INDEX IF EXISTS idx_expenses_household_incurred;
DROP TABLE IF EXISTS reports;

-- Restore original report tables from migration 00006
CREATE TABLE reports (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    household_id    UUID    NOT NULL REFERENCES households(id) ON DELETE CASCADE,
    month           INT     NOT NULL,
