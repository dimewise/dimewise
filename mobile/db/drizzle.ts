...
// Seed initial data
export const seedInitialData = async (db: ExpoSQLiteDatabase): Promise<void> => {
  try {
    ...
    const existingPayment = await db
      ...
      .where(and(eq(schema.paymentMethod.name, 'Cash'), eq(schema.paymentMethod.userId, userId)))
      .limit(1);
