...
export const convertCurrencyWithRate = (
	...
	if (toConfig.decimalPlaces === 0) {
		...
		return (
			Math.round(convertedAmount * 10 ** toConfig.decimalPlaces) /
			10 ** toConfig.decimalPlaces
		);
	}
};
 ...
 */
export const formatAmountWithCode = (
	amount: number,
	currency: CurrencyType,
): string => {
	const config = CURRENCY_CONFIG[currency];
