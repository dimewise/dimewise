...
export const exchangeRate = sqliteTable(
  'exchange_rate',
  {
    ...
    baseCurrency: text('base_currency', { enum: SUPPORTED_CURRENCIES }).notNull().default('USD'),
  ...
  (table) => [
    ...
    index('idx_exchange_rate_base_target').on(table.baseCurrency, table.targetCurrency),
  ],
);
