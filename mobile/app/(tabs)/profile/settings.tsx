import React, { useEffect, useState } from 'react';
import { ScrollView, View } from 'react-native';
import {
  Text,
  Button,
  useTheme,
  Appbar,
} from 'react-native-paper';
import { SafeAreaView } from 'react-native-safe-area-context';
import { router } from 'expo-router';
import { useTranslation } from 'react-i18next';
import { SUPPORTED_CURRENCIES, SUPPORTED_LANGUAGES, useUtilities } from '../../../storage';
import { Currency, Language } from '../../../storage';
import { useUserSettings, useCurrency } from '../../../utils/UserSettingsContext';
import DropdownBottomSheet, { DropdownButton, DropdownOption } from '../../../components/DropdownBottomSheet';

export default function SettingsScreen() {
  const [selectedCurrency, setSelectedCurrency] = useState<Currency>('USD');
  const [selectedLanguage, setSelectedLanguage] = useState<Language>('en');
  const [showCurrencyDropdown, setShowCurrencyDropdown] = useState(false);
  const [showLanguageDropdown, setShowLanguageDropdown] = useState(false);
  const [resetting, setResetting] = useState(false);
  const theme = useTheme();
  const { t } = useTranslation();
  const { currency, language, updateSettings } = useUserSettings();
  const { resetDatabaseVersion } = useUtilities();

  useEffect(() => {
    setSelectedCurrency(currency);
    setSelectedLanguage(language);
  }, [currency, language]);

  const handleSaveSettings = async () => {
    if (!selectedCurrency || !selectedLanguage) {
      return;
    }

    try {
      await updateSettings({
        currency: selectedCurrency,
        language: selectedLanguage
      });
    } catch (error) {
      console.error('Error saving settings:', error);
    }
  };

  const handleResetDatabase = async () => {
    setResetting(true);
    try {
      await resetDatabaseVersion();
      alert('Database version reset! Please restart the app to re-run migrations.');
    } catch (error) {
      console.error('Error resetting database:', error);
      alert('Error resetting database. Check console for details.');
    } finally {
      setResetting(false);
    }
  };

  // Convert currencies to dropdown options
  const currencyOptions: DropdownOption[] = SUPPORTED_CURRENCIES.map(curr => ({
    label: curr,
    value: curr,
    id: curr
  }));

  // Convert languages to dropdown options
  const languageOptions: DropdownOption[] = SUPPORTED_LANGUAGES.map(lang => ({
    label: lang.nativeLabel,
    value: lang.code,
    id: lang.code
  }));

  const renderLanguageDropdown = () => (
    <>
      <DropdownButton
        onPress={() => setShowLanguageDropdown(true)}
        selectedValue={selectedLanguage}
        options={languageOptions}
        placeholder={t('settings.selectLanguage')}
        label={t('settings.language')}
      />
      <DropdownBottomSheet
        visible={showLanguageDropdown}
        onDismiss={() => setShowLanguageDropdown(false)}
        options={languageOptions}
        onSelect={(value) => setSelectedLanguage(value as Language)}
        selectedValue={selectedLanguage}
        title={t('settings.selectLanguage')}
      />
    </>
  );

  const renderCurrencyDropdown = () => (
    <>
      <DropdownButton
        onPress={() => setShowCurrencyDropdown(true)}
        selectedValue={selectedCurrency}
        options={currencyOptions}
        placeholder={t('settings.selectCurrency')}
        label={t('settings.currency')}
      />
      <DropdownBottomSheet
        visible={showCurrencyDropdown}
        onDismiss={() => setShowCurrencyDropdown(false)}
        options={currencyOptions}
        onSelect={(value) => setSelectedCurrency(value as Currency)}
        selectedValue={selectedCurrency}
        title={t('settings.selectCurrency')}
      />
    </>
  );

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }} edges={[]}>
      <Appbar.Header style={{ backgroundColor: theme.colors.background }}>
        <Appbar.BackAction onPress={() => router.back()} />
        <Appbar.Content title={t('settings.title')} titleStyle={{ fontWeight: '700' }} />
      </Appbar.Header>

      <ScrollView style={{ flex: 1, backgroundColor: theme.colors.background }} contentContainerStyle={{ padding: 24 }}>
        {/* Language Settings Section */}
        <View style={{
          padding: 24,
          backgroundColor: theme.colors.surface,
          borderRadius: 8,
          borderWidth: 1,
          borderColor: theme.colors.outline,
          marginBottom: 16,
          shadowColor: '#000000',
          shadowOffset: { width: 0, height: 1 },
          shadowOpacity: 0.05,
          shadowRadius: 2,
          elevation: 1,
        }}>
          <Text variant="titleMedium" style={{ fontWeight: '600', marginBottom: 16, color: theme.colors.onSurface }}>
            {t('settings.language')}
          </Text>
          <View style={{ gap: 16 }}>
            {renderLanguageDropdown()}
          </View>
        </View>

        {/* Currency Settings Section */}
        <View style={{
          padding: 24,
          backgroundColor: theme.colors.surface,
          borderRadius: 8,
          borderWidth: 1,
          borderColor: theme.colors.outline,
          marginBottom: 16,
          shadowColor: '#000000',
          shadowOffset: { width: 0, height: 1 },
          shadowOpacity: 0.05,
          shadowRadius: 2,
          elevation: 1,
        }}>
          <Text variant="titleMedium" style={{ fontWeight: '600', marginBottom: 16, color: theme.colors.onSurface }}>
            {t('settings.defaultCurrency')}
          </Text>
          <View style={{ gap: 16 }}>
            {renderCurrencyDropdown()}
          </View>
        </View>

        {/* Save Settings Button */}
        <View style={{ marginBottom: 16 }}>
          <Button
            mode="contained"
            onPress={handleSaveSettings}
            disabled={selectedCurrency === currency && selectedLanguage === language}
            contentStyle={{
              paddingVertical: 12,
            }}
            labelStyle={{
              fontSize: 16,
              fontWeight: '600',
              letterSpacing: 0.25
            }}
            style={{
              borderRadius: 8,
              opacity: (selectedCurrency === currency && selectedLanguage === language) ? 0.6 : 1,
            }}
          >
            {(selectedCurrency === currency && selectedLanguage === language)
              ? t('settings.settingsUpdated')
              : t('settings.saveSettings')
            }
          </Button>
        </View>

        {/* Database Management Section */}
        <View style={{
          padding: 24,
          backgroundColor: theme.colors.surface,
          borderRadius: 8,
          borderWidth: 1,
          borderColor: theme.colors.outline,
          marginBottom: 16,
          shadowColor: '#000000',
          shadowOffset: { width: 0, height: 1 },
          shadowOpacity: 0.05,
          shadowRadius: 2,
          elevation: 1,
        }}>
          <Text variant="titleMedium" style={{ fontWeight: '600', marginBottom: 16, color: theme.colors.onSurface }}>
            {t('settings.databaseManagement')}
          </Text>
          <Text variant="bodyMedium" style={{
            color: theme.colors.onSurfaceVariant,
            marginBottom: 16,
            lineHeight: 20
          }}>
            {t('settings.resetDatabaseDescription')}
          </Text>
          <Button
            mode="outlined"
            onPress={handleResetDatabase}
            loading={resetting}
            disabled={resetting}
            contentStyle={{
              paddingVertical: 6,
            }}
            labelStyle={{
              fontSize: 14,
              fontWeight: '600',
              letterSpacing: 0.25
            }}
            style={{
              borderRadius: 6,
              borderColor: theme.colors.error,
            }}
            textColor={theme.colors.error}
          >
            {resetting ? t('settings.resetting') : t('settings.resetDatabaseVersion')}
          </Button>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
} 