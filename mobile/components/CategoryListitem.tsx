...
function isCategoryWithSpending(cat: CategoryWithSpending | Category): cat is CategoryWithSpending {
...
export const CategoryListItem = ({ category }: Props) => {
  ...
  const percentage = isCategoryWithSpending(category) ? category.percentage : undefined;
  ...
  const remainingFormatted =
    remaining !== undefined && remaining >= 0 ? formatAmount(remaining, currency) : null;
  const overBudgetFormatted =
    remaining !== undefined && remaining < 0 && overBudget !== undefined
    ...
    >
      <View style={{ gap: 20 }}>
        ...
        >
          ...
          >
            {spentFormatted ? `${spentFormatted} / ${budgetFormatted}` : budgetFormatted}
          </Text>
        </View>
